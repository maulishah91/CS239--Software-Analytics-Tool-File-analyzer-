...
object Main {
  def main(args: Array[String]) {
    val source = CSVFile("../resources/dataset/train/labeler_in.csv") ~> IDColumn(1);
    
    val tokenizer = {
      SimpleEnglishTokenizer() ~>            // tokenize on space and punctuation
      CaseFolder() ~>                        // lowercase everything
      WordsAndNumbersOnlyFilter() ~>         // ignore non-words and non-numbers
      MinimumLengthFilter(0)                 // take terms with >=0 characters
    }
    ...
    val labels = {
      ...
      TermMinimumDocumentCountFilter(1)     // filter labels in < 1 docs
    }
    ...
    TrainCVB0LabeledLDA(modelParams, dataset, output = modelPath, maxIterations = 10);
    // or could use TrainGibbsLabeledLDA(modelParams, dataset, output = modelPath, maxIterations = 15);
